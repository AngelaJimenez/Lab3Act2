...
class ServerProtocol:
    ...
    def __init__(self, numeroClientes, fileName, UDPServer):
    ...
    def enviarArchivoAlCliente(self, connection, thread_id):
        while True:
            try:
                ...
                self.udpServer.enviarArchivoUDP(thread_id, self.tamañoArchivo, self.fileName, self.bytesEnviados, self.paquetesEnviados)
    ...
    def allClientesReady(self, thread_id):
        ...
        all_ready = self.numeroClientes - self.clientesReady == 0
    ...
    def enviarArchivoUDP(self, thread_id):
        ...
        with open(File_path + self.fileName, 'rb') as file:
            ...
            self.paquetesEnviados[thread_id - 1] = self.tamañoArchivo / udpBUFFER_SIZE
    ...
    def log_info(self):
        if self.allCompleted():
            ...
            for n in range(self.numeroClientes):
                logging.info('Client{}: {} B'.format(n + 1, self.bytesEnviados[n]))
    ...
    def enviarArchivoUDP(self, thread_id, tamañoArchivo, fileName, bytesEnviados, paquetesEnviados):
        ...
        with open(File_path + fileName, 'rb') as file:
            ...
            while True:
                ...
                bytesEnviados[thread_id - 1] += int(b)

                progress.update(len(chunk))

            paquetesEnviados[thread_id - 1] = tamañoArchivo / udpBUFFER_SIZE
            print("Transmisión al cliente {} completada".format(thread_id))
            file.close()
...
def main():
    ...
    udpServer = UDPServer(IP, udpPort)
    s = ServerProtocol(nc, fileName, udpServer)
    s.run()
